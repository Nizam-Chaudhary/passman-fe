  ...
  return new Promise((resolve, reject) => {
    dbRequest.onsuccess = async function () {
      ...
      try {
        ...
        const exportedKey = await crypto.subtle.exportKey("jwk", encryptedKey);

        const transaction = db.transaction(INDEXED_DB_KEYS_STORE, "readwrite");
...
export async function getKeysFromIndexedDB(
  ...
  return new Promise((resolve, reject) => {
    dbRequest.onsuccess = function () {
      const db = dbRequest.result;
      const transaction = db.transaction(INDEXED_DB_KEYS_STORE, "readonly");
      const store = transaction.objectStore(INDEXED_DB_KEYS_STORE);
